...
enum FavoritePlaceSection {
    ...
    func getText() -> String {
        switch self {
        case .Fav:
            return Text.favoritePlace.localizedText
        case .Other:
            return Text.otherPlace.localizedText
        }
    }
}

struct FavoritePlaceVM {
    ...
    func generateUpdateViewModel(model: PlaceModel?) -> UpdatePlaceVM? {
        return UpdatePlaceVM(model: model)
    }
